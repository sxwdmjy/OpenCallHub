...
public class SipUdpDecoder extends MessageToMessageDecoder<DatagramPacket> {
    ...
    protected final Recycler<SipRequest> requestRecycler = new Recycler<SipRequest>() {
        @Override
    ...
    protected final Recycler<SipResponse> responseRecycler = new Recycler<SipResponse>() {
        @Override
        protected SipResponse newObject(Recycler.Handle<SipResponse> handle) {
            return new SipResponse(handle);
        }
    };
    protected static final Pattern START_LINE_PATTERN =
    ...
    private boolean isRequest(ByteBuf in) {
        // 检查是否为请求（同原逻辑）
        return !(in.getByte(0) == 'S' && in.getByte(1) == 'I' && in.getByte(2) == 'P');
    }
